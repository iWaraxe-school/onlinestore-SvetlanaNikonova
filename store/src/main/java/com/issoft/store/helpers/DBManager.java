...
public class DBManager implements IDBManager {
    ...
    public List<Product> getProductsForCategory(String categoryName) throws  SQLException {
        ...
        while (rs.next()) {
            productsFromDb.add(new Product(rs.getString("name"),rs.getString("category"),rs.getDouble("price"),rs.getDouble("rate")));
        }
    ...
    private static void createProductTable() throws SQLException {
        ...
        Statement stmt;
        stmt = connection.createStatement();
        stmt.execute(String.format("CREATE TABLE %s (id INTEGER IDENTITY PRIMARY KEY, name VARCHAR(50) UNIQUE, category_id INTEGER NOT NULL)", TABLE_PRODUCT_NAME,
