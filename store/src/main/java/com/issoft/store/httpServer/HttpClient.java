...
import static com.issoft.store.httpServer.AppHttpServer.PASSWORD;
import static com.issoft.store.httpServer.AppHttpServer.USERNAME;
...
public class HttpClient {
    ...
    public List<Product> getAllProducts() {
        HttpURLConnection connection = new HttpClient().getConnection("/products", HttpMethod.GET);
        ObjectMapper mapper = new ObjectMapper();
        try {
            return mapper.readValue(connection.getInputStream(), new TypeReference<List<Product>>() {
            });
        } catch (IOException e) {
            throw new RuntimeException(e.getMessage(), e);
        } finally {
